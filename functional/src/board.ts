...
export function create<T>(
  ...
  const board = createDimensionalArray(width, height, () => generator.next());
...
export function canMove<T>(
  ...
  if (
    !isValidRowIndex(first.row, board.height) ||
    !isValidColumnIndex(first.col, board.width) ||
    !isValidRowIndex(second.row, board.height) ||
    !isValidColumnIndex(second.col, board.width)
  ) {
    return false;
  }
...
function checkMatchesRecursive<T>(
  ...
  for (const match of getMatches(board)) {
    ...
    const refillEvent = createRefillEvent();
...
function createDimensionalArray<T>(
  width: number,
  height: number,
  getValue: () => T
) {
...
function isValidRowIndex(index: number, height: number) {
  return index >= 0 && index < height;
}

function isValidColumnIndex(index: number, width: number) {
